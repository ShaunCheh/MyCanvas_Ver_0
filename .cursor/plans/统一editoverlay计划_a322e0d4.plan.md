...
overview: 把 selection / rotate / crop chrome 收敛成单一 `editOverlay` 管线，作为一条独立子计划推进，让高亮框、四角方块、旋转手柄共享同一套 transform-aware 几何，并通过双写兼容逐步迁移平台层与控制器。
todos:
  - id: stage1-edit-overlay-model
    content: 定义统一 edit overlay 共享数据结构并在 snapshot 中双写兼容
    status: pending
  - id: stage2-renderer-selection-rotate
    content: 让 renderer 统一产出 selection/rotate edit overlay，并为角点补旋转语义
    status: pending
  - id: stage3-viewport-edit-overlay
    content: 迁移 iOS/macOS viewport 到 refreshEditOverlay，保证方块与高亮框整体旋转
    status: pending
  - id: stage4-controller-hit-testing
    content: 迁移 iOS/macOS controller 到统一 edit handle 命中与分发逻辑
    status: pending
  - id: stage5-crop-inline-session
    content: 把 crop overlay 与 inline edit state 收口到统一 payload/session
    status: pending
  - id: stage6-cleanup-regression
...
1. 阶段一：定义统一 `editOverlay` 骨架，并在 snapshot 中双写兼容。
...
1. 阶段二：让 shared renderer 统一产出 selection / rotate 的 `editOverlay`，并把角点旋转语义收口在 shared 层。
...
1. 阶段三：迁移 iOS / macOS viewport 到统一 `refreshEditOverlay()`，让四角方块与高亮框真正作为一个整体显示。
...
1. 阶段四：迁移 iOS / macOS controller 到统一 edit handle 命中与分发，避免 selection / rotate 各走一套入口。
...
1. 阶段五：把 crop overlay 与 inline edit state 收口到统一 payload / session，完成真正的“单一 edit overlay”。
...
1. 阶段六：删除旧 overlay 字段与重复 helper，并完成回归验证。
